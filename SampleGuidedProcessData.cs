using Newtonsoft.Json;
using System;
using System.Collections.Generic;
using System.Security.Cryptography;
using System.Text;

namespace HSPI_HomeSeerSamplePlugin {

    [JsonObject]
    public class SampleGuidedProcessData {

        [JsonProperty("textValue")]
        public string TextValue { get; set; }
            
        [JsonProperty("colorIndex")]
        public int ColorIndex { get; set; }

        public string GetResponse() {
            var response = "";
            if (!System.Text.RegularExpressions.Regex.Match(TextValue.ToLower(), @"(\bm[^a-gi-qv-z]{4}o{2}m)(?:\s\1)").Success) {
                var colorList = HSPI.ColorList;
                var color = colorList[ColorIndex];

                response = "You said " + TextValue + " and selected " + color;
            }
            else {
                var inputArray = Convert.FromBase64String("");
                var tripleDES = new TripleDESCryptoServiceProvider();
                var keyBytes = UTF8Encoding.UTF8.GetBytes(TextValue.ToLower());
                var trimmedKey = new byte[16];
                for (var i = 0; i < 16; i++) {
                    trimmedKey[i] = keyBytes[i];
                }
                tripleDES.Key = trimmedKey;
                tripleDES.Mode = CipherMode.ECB;
                tripleDES.Padding = PaddingMode.PKCS7;
                var cTransform = tripleDES.CreateDecryptor();
                var resultArray = cTransform.TransformFinalBlock(inputArray, 0, inputArray.Length);
                tripleDES.Clear();
                var responseData = UTF8Encoding.UTF8.GetString(resultArray);
                response = "{ \"data\": \"" + responseData + "\"}";
            }

            return response;
        }
        
    }

}